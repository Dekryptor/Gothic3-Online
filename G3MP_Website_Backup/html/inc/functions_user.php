...
function validate_password_from_uid($uid, $password, $user = array())
{
	...
	if($password == $user['password'])//if(salt_password(md5($password), $user['salt']) === $user['password'])
	{
		return $user;
	}
	else
...
function update_password($uid, $password, $salt="")
{
	...
	if(!$salt)
	{
		...
		{
			$salt = "NULL";//generate_salt();
		}
		$newpassword['salt'] = $salt;
	}

	// Create new password based on salt
	$saltedpw = $password;//salt_password($password, $salt);
...
{
	return $password;//return md5(md5($salt).$password);
}
