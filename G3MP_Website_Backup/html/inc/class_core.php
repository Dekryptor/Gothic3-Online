...
class MyBB {
	...
	function trigger_generic_error($code)
	{
		...
		{
			case "cache_no_write":
				$message = "Katalog z pamięcią podręczną (cache/) musi istnieć na serwerze i posiadać prawa do zapisu. Zmień jego uprawnienia CHMOD na 777, aby MyBB mogło zapisywać w nim pliki (tylko serwery Unix/Linux).";
				$error_code = MYBB_CACHE_NO_WRITE;
				break;
			case "install_directory":
				$message = "Katalog instalacyjny (install/) wciąż znajduje się na serwerze i nie został zablokowany. Aby przejść do swojego forum usuń ten katalog lub zablokuj go za pomocą pustego pliku o nazwie 'lock'.";
				$error_code = MYBB_INSTALL_DIR_EXISTS;
				break;
			case "board_not_installed":
				$message = "MyBB nie zostało jeszcze zainstalowane i skonfigurowane. Zrób to, zanim przystąpisz do przeglądania forum.";
				$error_code = MYBB_NOT_INSTALLED;
				break;
			case "board_not_upgraded":
				$message = "MyBB nie zostało jeszcze zaktualizowane. Zrób to, zanim przystąpisz do przeglądania forum.";
				$error_code = MYBB_NOT_UPGRADED;
				break;
			case "sql_load_error":
				$message = "MyBB nie może załadować obsługi serwera SQL. Odwiedź <a href=\"http://www.mybboard.pl\">Polski Support MyBB</a> aby uzyskać pomoc.";
				$error_code = MYBB_SQL_LOAD_ERROR;
				break;
			case "apc_load_error":
				$message = "Aby korzystać z systemu APC, należy go skonfigurować do pracy z PHP.";
				$error_code = MYBB_CACHEHANDLER_LOAD_ERROR;
				break;
			case "eaccelerator_load_error":
				$message = "Aby korzystać z systemu eAccelerator, należy go skonfigurować do pracy z PHP.";
				$error_code = MYBB_CACHEHANDLER_LOAD_ERROR;
				break;
			case "memcache_load_error":
				$message = "Twój serwer nie posiada wsparcia dla memcache.";
				$error_code = MYBB_CACHEHANDLER_LOAD_ERROR;
				break;
			case "memcached_load_error":
				$message = "Twój serwer nie posiada wsparcia dla memcached.";
				$error_code = MYBB_CACHEHANDLER_LOAD_ERROR;
				break;
			case "xcache_load_error":
				$message = "Aby korzystać z systemu Xcache, należy go skonfigurować do pracy z PHP.";
				$error_code = MYBB_CACHEHANDLER_LOAD_ERROR;
				break;
			default:
				$message = "MyBB napotkało błąd wewnętrzny. Odwiedź <a href=\"http://www.mybboard.pl\">Polski Support MyBB</a> aby uzyskać pomoc.";
